...
def get_openweather_forecast(lat, lon, api_key="02de0c63b48bcd13d425a73caa22eb81"):
    """Fetch weather forecast data from OpenWeather API for a given latitude and longitude."""
    lat = round(lat, 3)
    lon = round(lon, 3)
    ...
    try:
        ...
        weather_info = {
            "latitude": lat,
            "longitude": lon,
            "forecast": []  # Initialize an empty list to store forecast data
        }

        # Parse the forecast data
        for period in data["list"]:
            forecast_period = {
                "timestamp": pd.to_datetime(period["dt"], unit="s"),
                ...
                "wind_direction": period["wind"]["deg"]
            }
            weather_info["forecast"].append(forecast_period)
        
        print(f"✅ Retrieved forecast for ({lat}, {lon}): {len(weather_info['forecast'])} periods")
        return weather_info
    except requests.exceptions.RequestException as e:
        print(f"❌ Failed to retrieve forecast for ({lat}, {lon}): {e}")
        return None



def fetch_openweather_forecast_for_points(lat_lon_list, api_key="02de0c63b48bcd13d425a73caa22eb81"):
...
def fetch_weather_data(stations, start_date, end_date, api_key=TOKEN):
    """Fetch weather data for stations from OpenWeather API"""
    weather_data = []
    ...
    for station in stations:
        ...
        if lat and lon:
            ...
            params = {
                'lat': lat,
                'lon': lon,
                'dt': int(time.mktime(start_date.timetuple())),  # Start date converted to Unix timestamp
                'appid': api_key,
                'units': 'metric'
            }
            ...
            try:
                ...
                # Append the JSON response if the request is successful
                weather_data.append(response.json())
                
                # You may want to add a delay here to avoid rate limiting
                time.sleep(1)  # Wait 1 second between requests (adjust as needed)
            
            except requests.exceptions.HTTPError as err:
                if response.status_code == 401:
                    print(f"Error fetching weather data for station {station['id']}: Unauthorized (401). Check API key.")
                else:
                    print(f"Error fetching weather data for station {station['id']}: {err}")
            except Exception as e:
                print(f"Error fetching weather data for station {station['id']}: {e}")
    
    return weather_data

def fetch_weather_forecast_robust(flyway_bbox, resolution=0.25, forecast_dates=None, api_key=None):
    """
    ...
    """
    # Check for API key (previously token was used)
    if api_key is None:
    ...
    records = []
    forecast_dates_set = set(d.date() if hasattr(d, 'date') else d for d in forecast_dates) if forecast_dates else None
    ...
    for lat in lats:
        for lon in lons:
            try:
                ...
                # Fetching data for the point
                forecast_json = fetch_openweather_forecast_for_points([(lat, lon)], api_key=api_key)
                
                # The response is a list, so we need to loop over the list to handle multiple results
                for data in forecast_json:
                    periods = data.get("properties", {}).get("periods", [])
                    if not periods:
                        print(f"No forecast periods returned for point ({lat:.2f}, {lon:.2f}).")
                        continue

                    for period in periods:
                        period_start = pd.to_datetime(period["startTime"])
                        if forecast_dates_set and period_start.date() not in forecast_dates_set:
                            continue
                ...
                print(f"Error fetching forecast for point ({lat:.2f}, {lon:.2f}): {e}")
...
def main():
    ...
    forecast_data = fetch_weather_forecast_robust(flyway_bbox, resolution=0.25, forecast_dates=future_dates, api_key=TOKEN)
