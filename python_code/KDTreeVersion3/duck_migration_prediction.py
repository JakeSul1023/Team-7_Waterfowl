...
def get_nearest_noaa_location(lat, lon, token=None):
    ...
    try:
        ...
        props = data.get(f"properties", {})
        city = props.get(f"relativeLocation", {}).get(f"properties", {}).get(f"city")
        state = props.get(f"relativeLocation", {}).get(f"properties", {}).get(f"state")
        forecast_url = props.get(f"forecast", None)
...
def is_valid_noaa_point(lat, lon, token):
    url = f"https://api.weather.gov/points/{lat},{lon}"
    headers = {
        "User-Agent": "DuckMigrationApp/1.0",
        "token": token
...
def fetch_weather_forecast_robust(flyway_bbox, resolution= .25, forecast_dates=None, token="pKsYMZQINiCtWEbiUdvJHImQDqYlZmhu"):
...
def find_nearest_stations(lat, lon, tree, stations, k=3):
    ...
    # Get station info
    nearest_stations = [
    {**stations[idx], 'distance': dist} if 'name' in stations[idx] else {'name': 'Unknown', 'distance': dist}
    ...
    for i, stations in enumerate(nearest_stations):
        print(f"  {i+1}: {stations['name']} - {stations['distance']:.2f} km")
...
def calculate_weather_gradient(weather_rasters, timestamp, lat1, lon1, lat2, lon2, distance=50):
    ...
    current_weather = extract_weather_at_point(weather_rasters, all_timestamps, lat1, lon1)
    ahead_weather = extract_weather_at_point(weather_rasters, all_timestamps, ahead_lat, ahead_lon)
