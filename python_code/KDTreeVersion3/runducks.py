...
# Fetch station data first
stations = fetch_weather_data(valid_stations, start_date, end_date)

# Check if stations DataFrame is empty
if stations.empty:
    raise ValueError("No station data available. Check the source or parameters.")
...
# Convert stations DataFrame to a list of dictionaries for KD-tree
stations_list = stations[['station_id', 'latitude', 'longitude']].to_dict(orient='records')

# Build KD-tree using the fetched stations
...
point_weather = extract_weather_at_point(weather_rasters, timestamp, lat, lon)

# Example call
weather_gradients = calculate_weather_gradient(weather_rasters, all_timestamps, lat1, lon1, lat2, lon2, distance=50)
