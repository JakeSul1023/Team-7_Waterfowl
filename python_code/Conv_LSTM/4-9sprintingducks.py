...
def main():
    ...
    # Step 4: Fetch NOAA forecast for a sample point
    try:
        print(f"Fetching NOAA forecast for point ({lat}, {lon})...")
        forecast_json = fetch_noaa_forecast_for_point(lat, lon, TOKEN)
        print("Successfully fetched NOAA forecast")
    ...
    try:
        ...
        for station in stations_data.itertuples():
            ...
            station_dict = {
                'id': getattr(station, 'id', f"station_{len(valid_stations)}"),
    ...
    try:
        ...
        weather_data = fetch_weather_data(valid_stations[:50], start_date, end_date)
    ...
    try:
        ...
        forecast_data = fetch_weather_forecast_robust(flyway_bbox, resolution=3.0, forecast_dates=future_dates, token=TOKEN)
...
def generate_extended_forecasts(model, X_test, metadata_test, days_ahead=5):
    ...
    for i in range(len(X_test)):
        ...
        sequence = X_test[i:i+1]  # Keep batch dimension
        ...
        for day in range(1, days_ahead + 1):
            ...
            forecast = {
                ...
                'base_timestamp': meta['target_time'],
                'forecast_day': day,
                'forecast_timestamp': meta['target_time'] + timedelta(days=day),
            ...
            # Update the sequence for the next prediction (advanced implementation would
            # also update weather features based on forecasts)
