...
def load_duck_data(filepath="ShortTermSetData(Aug-Sept).csv"):
    ...
    try:
        ...
        if not timestamp_converted or duck_df['timestamp'].isna().sum() > len(duck_df) * 0.9:
            ...
            duck_df['timestamp'] = pd.date_range(start='2024-01-01', periods=len(duck_df), freq='1min')   #sample datetime for testing if failes
    except Exception as e:
...
def generate_extended_forecasts(model, X_test, metadata_test, days_ahead=7):
    ...
    for i in range(len(X_test)):
        ...
        base_time = meta['end_time']  # Use end_time as the starting point for forecasts
        ...
        for day in range(1, days_ahead + 1):
            ...
            # TODO: Update sequence with new weather features for next prediction
            # For simplicity, we're keeping the original sequence
            # In a production system, fetch forecasted weather for (pred_lat, pred_lon, base_time + day)
...
def main():
    ...
    if all_timestamps:
        ...
        # Fallback to recent dates if no timestamps
    ...
    if len(X) >= 10:  # Minimum number for meaningful splits
        ...
        for i, meta in enumerate(metadata_test):
            if abs((meta['end_time'] - last_timestamp).total_seconds()) < 24*3600:  # Within 1 day
